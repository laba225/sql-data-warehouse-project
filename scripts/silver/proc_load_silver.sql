...
BEGIN
	...
	BEGIN TRY
		...
		CASE WHEN UPPER (TRIM(cst_gndr)) = 'F' THEN 'Female'
			 ...
			 ELSE 'n/a'
		END AS cst_gndr, -- Normalize marital status value to readable format
		cst_create_date
		FROM (
		SELECT * ,
		...
		SELECT 
			...
			CASE UPPER(TRIM(prd_line))
				 ...
				 WHEN 'T' THEN 'Touring'
				 ELSE 'n/a'
			END AS prd_line, -- Map product line codes to descritpive values
		...
		SELECT 
		CASE WHEN cid LIKE'NAS%' THEN SUBSTRING(cid, 4, LEN(cid)) -- Remove 'NAS' prefix if presented
			 ELSE cid
		END AS cid,
		...
		PRINT '================================================';
		PRINT '>> Load Duration of silver Layer: ' + CAST(DATEDIFF(second, @start_time, @end_time) AS NVARCHAR) + ' seconds';
		PRINT '================================================';
